    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-start gap-3 min-w-0">
          <span className={`inline-block text-[11px] font-semibold px-2 py-0.5 rounded-full whitespace-nowrap flex-shrink-0
            ${COLORS[expense.category] || COLORS['Other']}`}>
            {expense.category}
          </span>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-sm font-semibold text-ink-700 dark:text-zinc-300 truncate">
                {expense.description || <span className="text-ink-300 dark:text-zinc-650 italic">No note</span>}
              </p>
              {expense.isRecurring && (
          ...
          <div className="flex items-center gap-1.5">
            {expense.isRecurring && (
              <HoverIcon onClick={handleStopRecurring} disabled={stopping}
                className="p-2 text-ink-400 dark:text-zinc-500 hover:text-amber-strong hover:bg-amber-soft dark:hover:bg-amber-950/20 rounded-xl transition-colors disabled:opacity-50"
            ...
            <HoverIcon onClick={() => onEdit(expense)}
              className="p-2 text-ink-400 dark:text-zinc-500 hover:text-sage hover:bg-sage-light dark:hover:bg-emerald-950/20 rounded-xl transition-colors"
              title="Edit">
            ...
            <HoverIcon onClick={handleDelete} disabled={deleting}
              className="p-2 text-ink-400 dark:text-zinc-500 hover:text-coral hover:bg-coral-soft dark:hover:bg-red-950/20 rounded-xl transition-colors disabled:opacity-50"
              title="Delete">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
